...
function applyOrientiation() {
	...
	if ((Ti.Gesture.orientation == Ti.UI.LANDSCAPE_LEFT)
		...
		$.commentScroller.height = deviceheight - 20 - 44 - 18 - 20 - 18 - 40
				- 40;
		...
		$.commentScroller.height = deviceheight - 200 - 20 - 44 - 18 - 20 - 18
				- 40 - 40;
...
if (args.thing) {
	...
	var labelText = ""
	labelText = labelText
			+ (args.thing.user == null ? "" : "Posted by "
					+ args.thing.user.name);
	labelText = labelText
			+ (createdDate == null ? "" : " on "
					+ createdDate.format('yyyy-MM-d'));
	labelText = labelText
			+ (args.thing.original_url == null ? "" : " from "
					+ Alloy.Globals.parseURL(args.thing.original_url));
...
function fetchComments() {
	...
	xhr.open('GET', Alloy.Globals.serverPath + '/api/things/' + thingId
			+ '/comments');
	xhr.send();
