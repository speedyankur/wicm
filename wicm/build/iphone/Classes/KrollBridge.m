...
NSString * Whichiscooler$ModuleRequireFormat = @"(function(exports){"
...
@implementation WhichiscoolerObject
...
{
	// create Whichiscooler global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Whichiscooler" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_whichiscooler = [[WhichiscoolerObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_whichiscooler];
	
	NSString *_whichiscoolerNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _whichiscoolerNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_whichiscooler valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:Whichiscooler$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.whichiscooler.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.whichiscooler.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
