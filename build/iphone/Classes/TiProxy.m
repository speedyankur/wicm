...
//Rare exceptions to indicate a bug in the _whichiscooler code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_whichiscooler = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_whichiscooler]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.whichiscooler.module"
